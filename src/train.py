...
def train_classifier():
    ...
    for epoch in range(50):
        ...
        training_batches, training_labels, validation_batches, validation_labels = split_dataset(
            data,
            cast(Any, torch).IntTensor([0] * positive_count + [1] * negative_count),
            0.75,
        )

        train_loss, train_acc = classifier.train(training_batches, training_labels)
        valid_loss, valid_acc = classifier.evaluate(validation_batches, validation_labels)
